...
export const CreatePolicie = () => {
	...
	return (
		<div className='overflow-y-scroll'>
			<form action='' onSubmit={handleSubmit(handleCreatePolicies)} className=''>
				<div className='py-1'>
					<InputComponent
						required
						name='name'
				...
				<div className='py-1'>
					<span className='mb-2'>Selecciona los modulos</span>
					{dataProcessinForCreatePolicies?.map((item) => (
						<div className='mt-2 pl-3' key={item._id}>
								...
								item.submodules.map((subItem) => (
									<div className=' ml-6 flex items-center mb-4' key={subItem._id}>
										<input
											...
											className='w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500  focus:ring-2'
											onChange={(event) =>
												handleCheckboxChangeSubItem(event, item._id, subItem._id)
											}
										/>
										<label
											htmlFor={subItem._id}
